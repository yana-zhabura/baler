...
#set page(width: 1000pt, height: auto)
...
- ```typc ()```: one column taking up the full available width,
- ```typc int```: number of columns, identical to setting all columns to `auto`,
- ```typst array``` of column sizes, where each column can be one of:
  - `length`: exact size,
  - ```typc (length, length)```: automatically determined size between the minimum and maximum allowed size,
  - ```typc auto```: automatically determined size between `50pt` and full width available.
